public class Main {
    public static void main(String[] args) throws InvalidPosition {
        ...
        board.place("f1", new Piece(PIECES.ROOK, COLORS.WHITE));
        board.place("d1", new Piece(PIECES.BISHOP, COLORS.WHITE));


        board.place("e8", new Piece(PIECES.QUEEN, COLORS.BLACK));
        board.place("a1", new Piece(PIECES.ROOK, COLORS.BLACK));
        ...
        try {
            System.out.println(Utils.isChekmate(board, COLORS.WHITE, Utils.convert("e1")));
        } catch (Exception e) {
